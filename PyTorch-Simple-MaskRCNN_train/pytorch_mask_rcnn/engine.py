...
def train_one_epoch(model, optimizer, data_loader, device, epoch, args, writer=None):
    ...
    tqdm_bar = tqdm(
                ...
                total = iters if args.iters >= 0 else len(data_loader)
            )
    ...
    for i, batch in enumerate(tqdm_bar):
        ...
        # warmup 구간이면 learning rate 선형 증가
        if num_iters <= args.warmup_iters:
            r = num_iters / args.warmup_iters
            for p in optimizer.param_groups:
                p["lr"] = r * args.lr_epoch
        ...
        # ------------------------------------
        # TensorBoard logging (iteration 단위)
        # ------------------------------------
        ...
        tqdm_bar.set_postfix(loss=f"{total_loss.item():.4f}")
        m_m.update(time.time() - S)
...
def evaluate(model, data_loader, device, args, generate=True):
    ...
    coco = dataset.coco
    print("DEBUG imgs:", len(coco.imgs))
    print("DEBUG anns:", len(coco.anns))
    print("DEBUG cats:", coco.cats)

    S = time.time()
    coco_evaluator.accumulate(results)
    print("accumulate: {:.1f}s".format(time.time() - S))

    print("DEBUG coco images:", len(dataset.coco.imgs))
    print("DEBUG coco annotations:", len(dataset.coco.anns))
    print("DEBUG coco categories:", dataset.coco.cats)
