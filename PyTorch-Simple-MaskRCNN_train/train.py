...
from pytorch_mask_rcnn.datasets.chncxr_dataset import CHNCXRDataset

def collate_fn(batch):
    """
    detection / segmentation 모델용 collate_fn
    - image, target 쌍을 그대로 리스트로 묶어줌
    - torchvision detection 공식 방식
    """
...
def main(args):
    ...
    # 기존 scratch preprocessed를 Torchvision Transform으로 교체
    train_tf = transforms.Compose([
        transforms.ToTensor(),
    ])

    dataset_train = CHNCXRDataset(
        root="data/chncxr",
        train=True,
        transforms=train_tf
    )
    ...
    # @variable d_test:
    #   평가용 데이터셋 객체
    #d_test = pmr.datasets(args.dataset, args.data_dir, "dental_part1", train=True) # set train=True for eval
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        writer = SummaryWriter(log_dir="runs/maskrcnn_lung)")
